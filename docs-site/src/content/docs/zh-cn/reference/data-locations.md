...
它是一个 SQLite 数据库（WAL 模式），由 sync 引擎写入：`sivtr sync` 显式执行一次同步，查询在 archive 比 `[sync].max_age_secs` 更旧时也会自动执行新鲜度同步，`pipe`/`run` 也会直接写入一次性 terminal capture。原生 Agent session 文件和 shell session log 仍是 source of truth，只有 sync 引擎会读取它们。可用 `SIVTR_DATA_DIR` 覆盖根目录。
